...
class Data_prep(object):
    ...
    def __init__(self):        
        ...
        self.frequecy_encoder_list  =  cPickle.load(bz2.BZ2File('/mnt/d/Gustavo/fraud-detection/frequecy_encoder_list.pbz2', 'rb')) # Opening encoder        
        ...
        self.frequecy_encoder_list = cPickle.load(bz2.BZ2File('/mnt/d/Gustavo/fraud-detection/frequecy_encoder_list.pbz2', 'rb'))
    ...
    def data_preparation(self, df2):
        ...
        self.frequecy_encoder_list.extend(X_train['nameDest'])
        
        nameDest_df = pd.DataFrame(data=self.frequecy_encoder_list, columns=['nameDest'])
        
        enc_nom_1 = (nameDest_df.groupby('nameDest').size()) / len(nameDest_df) # applying the encoder
        
        dict_enc = {} # dictionary of frequencies
        
        for i,it in zip(enc_nom_1.index, enc_nom_1):
            dict_enc[i] = it # populating the dictionary
            
            
        X_train['nameDest'] = X_train['nameDest'].map(dict_enc)
